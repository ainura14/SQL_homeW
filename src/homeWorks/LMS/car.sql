...
-- 1 Display only car’s brands & car’s models & car’s price (Таблицадан машиналардын бренд, модель жана бааларын чыгарыныз)
select cars.brand, cars.model, cars.price from cars;

-- 2 Display quantity of all cars (Таблицадан бардык машиналардын санын чыгарыныз)
select count(cars) from cars;

--3 Display all cars where brand is Hyundai (Hyundai брендиндеги бардык машиналарды чыгарыныз)
select * from cars where brand = 'Hyundai';

-- 4 Display all Red and Blue cars (Бардык кызыл жана кок машиналарды чыгарыныз)
select * from cars where color = 'Red' or color = 'Blue';

--5 Display all cars where issued year between 2000 and 2010 (Бардык 2000-жылдан 2010-жылга чейинки чыгарылган машиналарды чыгарыныз)
select * from cars where year_of_issue between 2000 and 2010;

-- 6 Display quantity of cars where car’s brand is Chevrolet (Chevrolet брендинде канча машина бар экенин чыгарыныз)
select * from cars where brand = 'Chevrolet';

--7 Display the average year of manufacture of cars (Машиналардын чыгарылган жылдарынын орточо маанисин чыгарыныз)
select avg(cars.year_of_issue) as average_year_of_manufacture from cars;

-- 8 Display the cars where brand equals to Audi, Toyota, Kia and Ford (Audi, Toyota, Kia жана Ford брендиндеги машиналдарды чыгарыныз)
select * from cars where brand = 'Audi' or brand = 'Toyota' or brand = 'Kia' or brand = 'Ford';

-- 9 Display cars where car’s model starts with ’T’ (Модели Т менен башталган машиналарды чыгарыныз)
select * from cars where brand like('T%');

-- 10 Display cars where car’s model ends with ‘e’(Модели “е” менен буткон машиналарды чыгарыныз)
select * from cars where brand like ('%e');

-- 11 Display brands where contains only 5 symbols (Бренди 5 символдон турган машиналарды чыгарыныз)
select * from cars  where length(brand) = 5 group by id;

-- 12 Display all amounts where car's brand is Mercedes-Benz (Бренди Mercedes-Benz болгон машиналардын жалпы суммасын чыгарыныз)
select brand, count(cars) as amounts_brand_Mercedes_Benz from cars where brand = 'Mercedes-Benz' group by brand;

-- 13 Display the most expensive car & the cheepest car (Эн кымбат жана эн арзан машинаны чыгарыныз)
select brand, cars.price as max_price from cars order by price desc limit 1;
select cars.brand, cars.price min_price from cars order by price limit 1;

-- 14 Display all cars where car's brand not TOYOTA (TOYOTA дан башка машиналарды чыгарыныз)
select * from cars where brand not like 'Toyota';

-- 15  Display 10 the most expensive cars (Эн кымбат 10 машинаны чыгарыныз)
select * from cars order by price desc limit 10;

-- 16 Display the newest cars between 5th to 15th (Эн жаны машиналардыдын 5 тен 15 ке чейинкисин чыгарыныз)
select * from cars where year_of_issue between 2005 and 2015;

-- 17 Display cars where car's year of issue not between 1995 and 2005 (1995 - жылдан 2005- ке чейинки машиналарадан башкаларын чыгарыныз)
select * from cars where year_of_issue not between 1995 and 2005;

--18 Display most cars in one color (Бир тустогу саны эн коп болгон машиналарды чыгаргыла)
select cars.brand, color,count(color) as count_colors from cars group by brand,color order by count_colors desc limit 1;
